...
export function sum(a, b) {
    const sum = a + b ;
    return [sum, `The sum of ${a} and ${b} is 11.`];
}
...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    ...
    let add = sum(a, b);
    let totalSum = sum(add[0], c);
    let mul = multiply(a, b);
    let totalProduct = multiply(mul[0], c);
    let StrOne = `${a} and ${b} and ${c} sum to ${totalSum[0]}.`;
    let StrTwo = `The product of ${a} and ${b} and ${c} is ${totalProduct[0]}.`;


    return [totalSum[0], totalProduct[0], StrOne, StrTwo];
}
...
export function sumArrayWithThreeNumbers(sumArr) {
    ...
    let total = sum(num[0], sumArr[2]);
    return [total[0], `${sumArr[0]},${sumArr[1]},${sumArr[2]} was passed in as an array of numbers, and ${total[0]} is their sum.`];
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    ...
    let total = multiply(num[0], multArr[2]);
    return [total[0], `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${total[0]}.`];
...
argument and returns an array whose first element is the product of those numbers, and the second
 element is a string that EXACTLY follows this example and concatenates a message using the arguments that were passed into the function:
